...
void LoadResources(CResourceManager* pResourceManager)
{
	if (pResourceManager)
	{
		// Textures
		...
		{
			pResourceManager
				->AddSound(Audio_SoundTrack, ".\\Resources\\Sounds\\BlastHornetSoundTrack.wav")
		...
		// Prefabs
		{
			// Effect Prefabs
			{
				...
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_Dash);
				...
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_DoubleJump);
				pPrefab->AddComponent<EffectAutoRemove>();

				pPrefab = pResourceManager->AddPrefab(Prefab_Effect_Smoke);
				pPrefab->AddComponent<CRenderer>()->SetZOrder(-20);
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_Smoke);
				pPrefab->AddComponent<EffectAutoRemove>();

				pPrefab = pResourceManager->AddPrefab(Prefab_Effect_SmokeDash);
				pPrefab->AddComponent<CRenderer>()->SetZOrder(-20);
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_SmokeDash);
				pPrefab->AddComponent<EffectAutoRemove>();

				pPrefab = pResourceManager->AddPrefab(Prefab_Effect_Explode);
				pPrefab->AddComponent<CRenderer>()->SetZOrder(-20);
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_Explode);
				pPrefab->AddComponent<EffectAutoRemove>();

				pPrefab = pResourceManager->AddPrefab(Prefab_Effect_Explode_Blue);
				pPrefab->AddComponent<CRenderer>()->SetZOrder(-20);
				pPrefab->AddComponent<CAnimator>()->AddAnimation(Animation_Effect_Explode_Blue);
				pPrefab->AddComponent<EffectAutoRemove>();
			...
			// Bullet Prefabs
			{
				...
				// Head Gunner Bullet
				{
					...
					pBullet->AddComponent<CAnimator>()->AddAnimation(Animation_HeadGunner_Bullet);
			...
			//Carry Aim Prefabs
			{
				CAnimation* anim = new CAnimation(Animation_Carry_Aim_Fly, Texture_EnemiesAndBosses, 8, 4, 80, true);
				{
					auto frames = anim->GetFrames();
					for (CAnimation::SFrame frame : frames)
					{
			...
			{
				CAnimation* anim = new CAnimation(Animation_MetaCapsule_Open, Texture_EnemiesAndBosses, 76, 19, 100, false);
				{
					auto frames = anim->GetFrames();
					for (CAnimation::SFrame frame : frames)
					{
...
{
	{
		...
		LoadResources(pResourceManager);				
	
		do
		{
			//Player
			CGameObject* pPlayer = new CGameObject(Player, { 7720, 1827 });//2160, 1129 Suriken || 45, 875 Start || 4370, 1100 Building || 7720, 1827 Blast
			{
				...
				pPlayer->AddComponent<CanBeAttacked>()->InitHealth(50);
				pPlayer->AddComponent<CAudioSource>();
				pPlayer->GetComponent<CAudioSource>()->AddSound(Audio_SoundTrack, true);// ->Play(AUDIO_SOUND_TRACK);
			...
			CGameObject* pHornet = new CGameObject("Blast Hornet", Vector2(7890, 1827));
			{
				//Animation
				{
					...
					auto anim = new CAnimation(Animation_BlastHornet_StartAttacking, Texture_Blast_Hornet, 3, 13, 100, false);
			...
			//Genjibo
			{
				...
				auto anim = new CAnimation(Animation_Shurikein_WasHit, Texture_EnemiesAndBosses, 180, 1, 50, true);
			...
			pPlayer->GetComponent<CAudioSource>()->Play(Audio_SoundTrack);

			// New
			CGameLogic* pGameLogic = CGameLogic::Instantiate("Game Logic");
			pGameLogic->AddComponent<GameLogicController>();
