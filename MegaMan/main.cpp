...
void LoadResource(CResourceManager* pResourceManager)
{
	// Textures
	{
	...
	{
		auto anim = new CAnimation(Animation_MegaManX_Dash, Texture_MegaManX, 81, 1, 100, false);
	...
	//Animation Effect
	{
		...
		auto anim = new CAnimation(Animation_Effect_Dash, Texture_MegaManX_Effect, 22, 8, 70, true);
		{
...

	{
		...
		auto anim = new CAnimation(Animation_Effect_Explode, Texture_MegaManX_Explosions_Effect, 0, 6, 100, false);
	...
	//Bullet Prefabs
	{
		...
		//Helit missle
		{
			...
			helitMisslePrefab->AddComponent<CanAttacked>()->InitDamage(20);
		}
		...
		//Carry Aim Prefabs
		{
			auto anim = new CAnimation(Animation_Carry_Aim_Fly, Texture_EnemiesAndBosses, 8, 4, 80, true);
			{
				auto frames = anim->GetFrames();
				for (CAnimation::SFrame frame : frames)
				{
		...
		{
			auto anim = new CAnimation(Animation_MetaCapsule_Open, Texture_EnemiesAndBosses, 76, 19, 100, false);
			{
				auto frames = anim->GetFrames();
				for (CAnimation::SFrame frame : frames)
				{
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	LoadResource(pResourceManager);
	

	do
	{
		CGameObject* pPlayer = new CGameObject(Player, Player_Init_Position);
		{			
		...
		//UI
		{
			const Vector2 camSize = pScene->GetMainCamera()->GetComponent<CCamera>()->GetSize();
			CGameObject* pHealthBar = new CGameObject("Health Bar");
			pHealthBar->GetComponent<CTransform>()->SetParent(pScene->GetMainCamera())->Set_Position({
			...
			/*{
				auto anim = new CAnimation(Animation_Ready, Texture_Ready, 0, 11, 100, false);
				...
				pReady->GetComponent<CTransform>()->SetParent(pScene->GetMainCamera())->Set_Position({ -39/2,-13/2 }, false);
		...
		CGameObject* pHornet = new CGameObject("Blast Hornet", Vector2(7890, 1827));
		{
			...
			//Animation
			{
				...
				auto anim = new CAnimation(Animation_BlastHornet_StartAttacking, Texture_Blast_Hornet, 3, 13, 200, false);
		...
		CGameObject *pShurikein = new CGameObject("Shurikein", Vector2(2477, 1181));
		{
			...
			pShurikein->AddComponent<CanBeAttacked>()->InitHealth(100);
